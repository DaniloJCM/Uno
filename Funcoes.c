...
int trocaCorEstaNoDescarte = 0;
char corEscolhida = 'w';    // w de branco, so para representar que nao foi escolhida nenhuma cor 
...
void jogadaDe(Descarte *descarte, Jogador *jog){
    ...
    int posicao = lerNumero(1, jog->qtasCartas + 1);

if(posicao != jog->qtasCartas + 1){
...
int cartaPodeSerJogada(Carta *c, Descarte *descarte){
    ...
    if(c->cor == 'p'){
        ...
        int corTrocada = trocaCorEstaNoDescarte;
        if(corTrocada){
            if(c->cor == corEscolhida){ 
                podeJogar = 1;
                trocaCorEstaNoDescarte = 0;
                corEscolhida = 'w';
            }
            else{
                podeJogar = 0;
            ...
            if(c->simbolo == descarte->topo->simbolo || c->cor == descarte->topo->cor){
                podeJogar = 1;
            }
            else{
                podeJogar = 0;
...
void imprimeTopoDescarte(Descarte *d){
    ...
    if(!trocaCorEstaNoDescarte){
        imprimeCarta(d->topo); 
    }
    else{
        imprimeCor(corEscolhida);
        imprimeCarta(d->topo);
        printf(white);
    } 
    printf("\n\n");
...
void trocaCor(){
    ...
    else if(qualEscolhida == 2){  
        printf("Verde!");
        corEscolhida = 'g';
    }
    else if(qualEscolhida == 3){
        printf("Azul!");
        corEscolhida = 'b';        
    }
    else {
